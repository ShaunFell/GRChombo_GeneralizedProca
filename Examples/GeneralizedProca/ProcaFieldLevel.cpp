...
void ProcaFieldLevel::initialData()
{
    ...
    BoxLoops::loop(make_compute_pack(SetValue(0.),
                                     ...
                                     InitialProcaData(m_p.initialdata_params,
                                                      m_p.potential_params,
                                                      m_p.kerr_params, m_dx)),
                   m_state_new, m_state_new, INCLUDE_GHOST_CELLS);
    ...
    {
        BoxLoops::loop(ExcisionProcaEvolution<ProcaFieldWithPotential>(
                           m_dx, m_p.center, m_p.inner_r),
    ...
    if (m_p.nan_check)
    {
        BoxLoops::loop(NanCheck(m_dx, "NaNCheck in initial data: "),
                       m_state_new, m_state_new, EXCLUDE_GHOST_CELLS,
                       disable_simd());
    }
...
void ProcaFieldLevel::prePlotLevel()
{
    ...
    ProcaConstraint<ProcaPotential> proca_constraint(
        m_dx, m_p.potential_params.mass, m_p.proca_params.vector_damping,
        potential);
    EffectiveMetric<ProcaPotential> proca_eff_met(
        m_dx, m_p.potential_params.mass, m_p.proca_params.vector_damping,
        potential);
                                      ...
                                      const double a_time)
{
    ...
    {
        MatterCCZ4RHS<ProcaFieldWithPotential, MovingPunctureGauge,
                      FourthOrderDerivatives>
            my_ccz4_matter(proca_field, m_p.ccz4_params, m_dx, m_p.sigma,
...
void ProcaFieldLevel::specificUpdateODE(GRLevelData &a_soln,
...
void ProcaFieldLevel::preTagCells()
{
    ...
    /*     //fill all ghosts
        ...
        ProcaPotential potential(m_p.potential_params);
        ProcaFieldWithPotential proca_field(potential, m_p.proca_params);
        ProcaConstraint<ProcaPotential> proca_constraint(m_dx,
       m_p.potential_params.mass, m_p.proca_params.vector_damping, potential);

        //compute Hamiltonian and Guass diagnostics on each cell of current
       level as these are required for tagging BoxLoops::loop(
            make_compute_pack(
                MatterConstraints<ProcaFieldWithPotential>(proca_field,
                                                            m_dx, m_p.G_Newton,
                                                            c_Ham,
                                                            Interval(c_Mom1,
       c_Mom3), c_Ham_abs_sum, Interval(c_Mom_abs_sum1,c_Mom_abs_sum3)
                                                            ),
    ...
    const FArrayBox &current_state_diagnostics)
{
    ...
    BoxLoops::loop(FixedGridsTaggingCriterion(
                       m_dx, m_level, m_p.grid_scaling * m_p.L, m_p.center),
...
void ProcaFieldLevel::specificPostTimeStep()
{
    ...
    if (m_p.AH_activate && m_level == m_p.AH_params.level_to_run)
    {
        ...
        // If we perform excision of matter variables using found Apparent
        // Horizon
        if (m_p.excise_with_AH)
        {
            pout() << "AH solved. Excising inside AH" << endl;
            double num_AH_points{
                (double)(m_bh_amr.m_ah_finder.get(0)->m_params.num_points_u) *
                (double)(m_bh_amr.m_ah_finder.get(0)->m_params.num_points_v)};
            ...
            BoxLoops::loop(ExcisionProcaEvolutionWithAH<ProcaFieldWithPotential,
                                                        decltype(AH_Interp)>(
                               m_dx, m_p.kerr_params.center, AH_Interp,
                               num_AH_points, 0.97),
                           m_state_new, m_state_new, EXCLUDE_GHOST_CELLS,
                           disable_simd());
        };
    ...
    bool first_step =
        (m_time == m_dt); // is this the first call of posttimestep?
    ...
    if (m_p.activate_extraction)
    {
        ...
        if (calculate_weyl)
        {
            ...
            BoxLoops::loop(MatterWeyl4<ProcaFieldWithPotential>(
                               proca_field, m_p.extraction_params.center, m_dx,
                               m_p.formulation, m_p.G_Newton),
            ...
            BoxLoops::loop(
                ExcisionDiagnostics(m_dx, m_p.center, 0.0,
                                    m_p.extraction_params.extraction_radii[1]),
                m_state_diagnostics, m_state_diagnostics, SKIP_GHOST_CELLS,
                disable_simd());
