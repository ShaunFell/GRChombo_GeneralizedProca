...
class KerrQI
{
  ...
  public:
    KerrQI(params_t a_params, double a_dx) : m_dx(a_dx), m_params(a_params)
    ...
    template <class data_t>
    void compute_kerr(Tensor<2, data_t> &spherical_g,
                              Tensor<2, data_t> &spherical_K,
                              Tensor<1, data_t> &spherical_shift,
                              data_t &kerr_lapse,
    ...
    template <class data_t>
    data_t excise( const Cell<data_t> &current_cell) const
    {
        // black hole params - mass M and spin a
        const data_t M = m_params.mass;
        const data_t a = m_params.spin;
        const data_t a2 = a * a;

        //work out where we are on grid
        const Coordinates<double> coords(current_cell, m_dx, m_params.center);

        //horizon radius in Boyer-Lindquist coords
        const data_t r_plus = M + sqrt(M * M - a2);

        //horizon radius in Quasi-Isotropic coords is 1/4 of Boyer-Lindquist
        const data_t r_plus_QI { r_plus/4.0 };

        //Distance to horizon as fraction. Less than one means inside horizon
        const data_t horizon_distance { coords.get_radius() / r_plus_QI };
